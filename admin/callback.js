...
// admin-src/callback.js
var formSection = document.getElementById("password-form-section");
...
async function run() {
  ...
  try {
    ...
    showMessage("Lien invalide ou expir\xE9 : " + (err && err.message ? err.message : "erreur inconnue"), true);
    return;
  }
  if (!result) {
...
form.addEventListener("submit", async (e) => {
  ...
  try {
    ...
    statusEl.textContent = err && err.message ? err.message : "Impossible de d\xE9finir ce mot de passe.";
