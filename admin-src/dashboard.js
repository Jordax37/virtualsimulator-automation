...
async function openBatchDetail(batchId) {
  ...
  try {
    ...
    el.innerHTML = `<table><thead><tr><th></th><th>Statut</th><th>Étape</th><th>Véhicule / URL source</th><th>Tentatives</th><th>Créé le</th><th></th></tr></thead><tbody>
      ${vehicle_jobs
        .map((j) => {
  ...
  try {
    ...
    if (vd) {
      ...
      if (vd.photos && vd.photos.length) {
        html += `<div class="vehicle-gallery">${vd.photos
          .map((p) => `<a href="${escapeHtml(p)}" target="_blank" rel="noopener"><img src="${escapeHtml(p)}" loading="lazy" alt="" /></a>`)
          .join("")}</div>`;
      }
